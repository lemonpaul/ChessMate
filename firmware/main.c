...
const PROGMEM uint8_t ascii_char[16] = {'1','2','3','4','5','6','7','8',
                                        'a','b','c','d','e','f','g','h'};
const PROGMEM uint8_t     digits[16] = {_1, _2, _3, _4, _5, _6, _7, _8, 
...
void Refresh_display()
{
    ...
    { 
        ...
        if (CONTROL)                                   // count how many cycles (2,6ms each)
        {                                              // button CLR/NEW remain pressed
        ...
        else                                            // button ENTER is released
        {
            ...
            if (temp > treshold_short_press)
            {
                ...
                switch (last_key)
                {
                     case 0b00010000:                   // Key 1
                         KeyStatus |= 0;
                         break;
                     case 0b00001000:                   // Key 2
                         KeyStatus |= 1;
                         break;
                     case 0b00000010:                   // Key 3
                         KeyStatus |= 2;
                         break;
                     ...
                     case 0b00100000:                   // Key 5
                         KeyStatus |= 4;
                         break;
                     case 0b00000100:                   // Key 6
                         KeyStatus |= 5;
                         break;
                     case 0b00000001:                   // Key 7
                         KeyStatus |= 6;
                         break;
                     case 0b01000000:                   // Key 8
                         KeyStatus |= 7;
                         break;
                     default:                           // more than one key pressed simultaneously
    ...
    for (;;)
    {
        switch (LilyState)
        {
            case _Startup_Screen:
                Display[0] = _P;                                    // Now is fixed but can be
                                                                    // animated
                Display[1] = _L;                                    // but now I am simply
                                                                    // overwriting
                Display[2] = _A;
                Display[3] = _Y;
                ...
                {
                    KeyStatus = 0;                                  // If that's the case start
                                                                    // a new game
                    LilyState = _New_Game;
                    RL = pgm_read_word(&difficult_level[(i&0x07)]); // Configure recursion
                                                                    // (difficulty) level
                }
                break;

            case _New_Game:
                Display[0] = _D;                                // Update display. This message can
                                                                // be animated
            ...
            case _Human_Move:
                ...
                i = KeyStatus;                                                  // Save Key Events
                if (KeyStatus & (1<<NEW_EVENT))                                 // Check for a NEW
                                                                                // event
                {
                    LilyState = _New_Game;                                      // If that's the
                                                                                // case start
                                                                                // a new game
                    RL = pgm_read_word(&difficult_level[(i & 0x07)]);           // set difficulty
                                                                                // level
                    KeyStatus = 0;
                    break;
                }
                ...
                {
                    if ((c[0] == 97 && c[1] == 32 && (RL<difficult_level[4])))
                    {
                        c[0] = 10;                                              // then tell Lily
                                                                                // to play for you
                    }
                    Display[0] = _DP;  
                    PORTB = 0xff;
                    ...
                    {                                                           // legal
                        ...
                        {
                            LilyState = _Human_Move;
                            First_key = 1;                                      // for clear
                                                                                // display
                            KeyStatus = 0;
                        }
                        else
                        {
                            LilyState = _Lily_Move;                             // Send your move
                                                                                // to Lily
                            KeyStatus = 0;
                        }
                        break;
                ...
                if (KeyStatus & (1<<CLR_EVENT) && !(KeyStatus & (1<<CLR_LONG))) // clear display
                {                                                               // and decrement
                                                                                // pointer
                                                                                // according with
                                                                                // position
                    switch(digit_now & 0x03)
                    {
                        ...
                        case 0:
                            ...
                            break;
                        case 3:
                            if (c[digit_now] == 32)                             // if haven't keyed
                            {                                                   // yet on the last
                                                                                // position
                                digit_now--;                                    // return to
                                                                                // previous
                            ...
                            {
                                c[digit_now] = 32;                              // otherwise clear
                                                                                // current position
                                                                                // only
                                Display[digit_now] = _SPC;
                                break;
                ...
                if (KeyStatus & (1<<KEY18_EVENT) )
                {
                    ...
                    switch(digit_now & 0x03)
                    {
                        case 0:
                        case 2:
                            c[digit_now] = pgm_read_byte(ascii_char+i+8);
                            Display[digit_now] = pgm_read_byte(digits+i+8);
                            break;
                        case 1:
                        case 3:
                            c[digit_now] = pgm_read_byte(ascii_char+i);
                            Display[digit_now] = pgm_read_byte(digits+i);
                            break;
                    }
                    if (digit_now < 3)                                          // limit to the
                    {                                                           // last digit
                        digit_now++; 
                    }
                    KeyStatus = 0;
            ...
            case _Lily_Move:
                ...
                /* Check for Terminated game (bit 7 is set). In this case playlily returns new */
                /*    difficulty level                                                         */
                if (cm & 0x80 )
                {
                    i = cm & 0x07;
                    RL = pgm_read_word(&difficult_level[(cm & 0x07)]);      // Configure recursion
                                                                            // (difficulty) level
                    LilyState = _New_Game;
                    KeyStatus = 0;
                    break;
                ...
                {
                    LilyState = _Lily_Move;                                 // Send your move to
                                                                            // Lily
                    KeyStatus = 0;
                }
                break;
